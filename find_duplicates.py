...
def get_metadata_tags(file_path):
    ...
    try:
        ...
        logger.warning(f"Could not process file {os.path.basename(file_path)}: {e}")
        return None, None
    except Exception as e:
        logger.error(f"Error reading metadata for {os.path.basename(file_path)}: {e}")
        return None, None
...
def print_duplicate_results(duplicate_groups, mode):
    ...
    logger.info(f"Found {len(duplicate_groups)} folder(s) with potential duplicates (Mode: {mode})\n")
    
    folder_count = 0
    for folder, groups in duplicate_groups.items():
        folder_count += 1
        logger.info(f"[{folder_count}] Folder: {folder}")
...
def print_count_results(folders):
    ...
    logger.info("\n--- Scan Complete ---")
    if not folders:
        logger.info("No folders with more than one audio file found.")
        return

    logger.info(f"Found {len(folders)} folder(s) containing more than one audio file.\n")
    
    for i, (folder, files) in enumerate(folders.items()):
        logger.info(f"[{i+1}] Folder: {folder} ({len(files)} files)")
        for f in files:
            logger.info(f"  - {f}")
        logger.info("---" * 20)
...
def main():
    try:
        ...
        if not os.path.isdir(target_dir):
            logger.error(f"The specified directory '{target_dir}' does not exist.")
            return
